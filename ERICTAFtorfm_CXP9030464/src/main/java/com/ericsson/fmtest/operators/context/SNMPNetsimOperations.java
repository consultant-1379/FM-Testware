...
public class SNMPNetsimOperations {

	private static final Logger LOGGER = Logger
			.getLogger(SNMPNetsimOperations.class);
	 ...
	 *            Node name in the simulation of a Netsim Server
	 */
	public String raiseTestAlarm(String simulationName, String neName, String neType) {
		String selectCmd = NetsimAPIOperatorAPIGetter.SELECT;
		selectCmd = selectCmd.concat(neName);
		String startNECmd = NetsimAPIOperatorAPIGetter.START;
	...
	public void raiseTestSNMPClearAlarm(String simulationName, String neName,
			String neType,String alarmId) {
		String selectCmd = NetsimAPIOperatorAPIGetter.SELECT;
		selectCmd = selectCmd.concat(neName);
		String startNECmd = NetsimAPIOperatorAPIGetter.START;
		String ceaseAlarmIdCmd = NetsimAPIOperatorAPIGetter.CEASEID;
		ceaseAlarmIdCmd = ceaseAlarmIdCmd.concat(alarmId);
		ceaseAlarmIdCmd = ceaseAlarmIdCmd.concat(";");
		String result = handler.executeCommandWithSimulation(simulationName,
				selectCmd, startNECmd, ceaseAlarmIdCmd);
		LOGGER.debug("Result of the ceaseAllAlarms command execution is:"
				+ result);	
	}
