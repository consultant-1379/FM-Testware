...
import com.ericsson.fmtest.operators.context.NetsimAPIOperator;
import com.ericsson.oss.fmservice.api.beans.Alarm;

public class NetsimOperator implements GenericOperator {

	NetsimAPIOperator netsimAPIOperator = new NetsimAPIOperator();

	private static final Logger LOGGER = Logger.getLogger(NetsimOperator.class);
	...
	public String raiseAlarm(String fdn, String neType, String simulationName,
			String neName, String spText, String severity) {

		String retResult = null;
		try {
			retResult = netsimAPIOperator.raiseAlarm(fdn, neType,
					simulationName, neName, spText, severity);
		} catch (Exception e) {
	...
	public String raiseAlarm(String fdn, String neType, String simulationName,
		...
		try {
			retResult = netsimAPIOperator.raiseAlarm(fdn, neType,
					simulationName, neName, spText, pcText, severity);
		} catch (Exception e) {
			LOGGER.error(e.getStackTrace().toString());
		}
		return retResult;
	...
	public String raiseDefaultAlarm(String simulationName, String neName, 
	         String neType) {
		String retResult = null;
		try {
	...
	public List<String> sendNetsimAlarms(String testData,
		...
		try {
			...
			outputAlarmList = NetsimAPIOperator.getBulkAlarmFromDB(data[1],
					specificPrb);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<String> sendNetsimAlarms(String testData,
		...
		for (Map.Entry<String, String> entry : spattributesMap.entrySet()) {
			...
			if (key != null) {
				...
				String sent = raiseAlarm(data[1], data[0], data[2], data[3],
						key.split("\\$")[1], pcKey.split("\\$")[1], severity);
				if (sent.contains("ERROR")) {
					LOGGER.error("ERROR: Sending alarm through NETSIM FAILED for SPText : "
							+ value);
				}
		...
		try {
			com.ericsson.fmtest.operators.Utils.sleep(10);
			outputAlarmList = NetsimAPIOperator.getAlarmFromDB(data[1],
	...
	public List<String> sendNetsimAlarms_PC(String testData,
		...
		try {
			com.ericsson.fmtest.operators.Utils.sleep(10);
			outputAlarmList = NetsimAPIOperator.getBulkPCAlarmFromDB(data[1],
					probableCause);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String expectedAlarmResult(String fdn) {
		...
		alarm.setFdn(fdn);
		if (!fdn.contains("ManagedElement=1"))
			alarm.setFdn(fdn + ",ManagedElement=1");
